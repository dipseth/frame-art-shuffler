...
def _filename_to_point_id(filename: str) -> int:
    """Deterministic 53-bit point ID for a filename (MD5-derived).

    Used so the same filename always maps to the same Qdrant point across
    runs — enables O(1) retrieve by filename and lets discover() reference
    library images directly without a separate lookup.
    """
    return int(hashlib.md5(filename.encode()).hexdigest(), 16) % (2**53)
...
class QdrantArtStore:
    ...
    def upsert_image(
        ...
        *,
        ...
        point_id = _filename_to_point_id(filename)
    ...
    def discover(
        ...
        *,
        ...
        """Qdrant Discover: rank by similarity to target, shaped by (pos, neg) pairs.

        Exactly one of target_filename / target_text must be provided.
        positives/negatives are lists of filenames; unknown ones are skipped.
        Returns the same payload+score dicts as search_full.
        """
        if (target_filename is None) == (target_text is None):
            raise ValueError("Provide exactly one of target_filename or target_text")

        positives = positives or []
        negatives = negatives or []

        # Resolve target. For text we embed inline; for an image we use its
        # MD5-derived point ID and let Qdrant fetch the stored vector.
        if target_text is not None:
            target = _jina_embed_texts([target_text], self._jina_key)[0]
        else:
            target = _filename_to_point_id(target_filename)
            if self.find_by_filename(target_filename) is None:
                raise ValueError(f"Target image not found in collection: {target_filename!r}")

        # Resolve positive / negative example IDs, skipping unknowns.
        pos_ids = [
            _filename_to_point_id(fn)
            for fn in positives
            if self._exists(fn, "positive")
        ]
        neg_ids = [
            _filename_to_point_id(fn)
            for fn in negatives
            if self._exists(fn, "negative")
        ]

        # Build context pairs by zipping positives × negatives, cycling the
        # shorter list so every entry contributes a constraint. Empty either
        # side → no context, which Qdrant treats as plain target similarity.
        pairs: list[tuple[int, int]] = []
        if pos_ids and neg_ids:
            n = max(len(pos_ids), len(neg_ids))
            for i in range(n):
                pairs.append((pos_ids[i % len(pos_ids)], neg_ids[i % len(neg_ids)]))

        # Lazy-import the discover models — name layout settled in qdrant-client
        # 1.10+ (which we already require via query_points usage).
        from qdrant_client.models import ContextPair, DiscoverInput, DiscoverQuery

        context = [ContextPair(positive=p, negative=n) for p, n in pairs]
        query = DiscoverQuery(
            discover=DiscoverInput(target=target, context=context)
        )
    ...
    def _exists(self, filename: str, role: str) -> bool:
        if self.find_by_filename(filename) is not None:
            return True
        _LOGGER.warning(
            "discover: skipping unknown %s example %r (not in collection)",
            role, filename,
        )
        return False
